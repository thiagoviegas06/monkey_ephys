...
class SessionDataPhase2:
    """Class to hold Phase 2 session data and paths."""
    def __init__(self, data_path, is_train=True):
        ...
        print(f"Found {len(self.sessions)} sessions in {split_dir}.")
        
    def generate_session_obj(self):
        session_objects = []
        for session_id in self.sessions:
            session_objects.append(SessionObjPhase2(self.data_path, session_id, self.is_train))
        return session_objects

class SessionObjPhase2:
    def __init__(self, data_path, session_id, is_train=True):
        self.data_path = data_path
