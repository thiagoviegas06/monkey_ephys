...
class KinematicDecoderTransformer(nn.Module):
    ...
    """
    def __init__(self, d_model=64, window_size=200, num_channels=96,
    ...
    def forward(self, transformer_repr):
        ...
        x_flat = x_weighted.reshape(B_W, -1)  # (B*W, C*d_model)
        x_proj = self.channel_proj(x_flat)  # (B*W, d_model)

        # 3. Reshape for temporal attention: (B, W, d_model)
        x_temporal = x_proj.reshape(B, self.window_size, d_model)
